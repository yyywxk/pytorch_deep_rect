#!/usr/bin/env python
# -*- coding: utf-8 -*-
# @Time    : 2022/5/25 16:01
# @Author  : yyywxk
# @File    : spatial_transform_local_feature.py
...
def transformer(U, theta, grid_h, grid_w, cuda=False, name='SpatialTransformer', **kwargs):
    grid_w = grid_w
    grid_h = grid_h
    ...
    def _transform3(theta, input_dim, cuda=False):
        '''

        :param theta: mesh B X 2 X (U+1) X (V+1)
        :param input_dim: feature B X C X H X W
        :param cuda:
        :return:
        '''
        ...
        # warp_image = input_transformed.reshape(num_batch, height, width, num_channels)
        warp_image = input_transformed.view(num_batch, height, width, -1)
        # warp_mask = mask_transformed.reshape(num_batch, height, width, num_channels)

        return warp_image.permute(0, 3, 1, 2)

    warp_image = _transform3(theta, U, cuda=cuda)
    return warp_image
...
if __name__ == '__main__':
    ...
    img = F.interpolate(img, size=(24, 32), mode='nearest')
    ...
    feature_warp = transformer(img, ori_pt, grid_w=grid_w, grid_h=grid_h, cuda=False)

    test_warp_image = (feature_warp[0] + 1.0) * 127.5
