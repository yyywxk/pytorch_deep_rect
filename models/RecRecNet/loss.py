...
def cal_inter_grid_loss(mesh, grid_w=8, grid_h=8):
    ...
    cos_w = torch.sum(w_edges[:, :, 0:grid_w - 1, :] * w_edges[:, :, 1:grid_w, :], 3) / (
                torch.sqrt(torch.sum(w_edges[:, :, 0:grid_w - 1, :] * w_edges[:, :, 0:grid_w - 1, :], 3)) * torch.sqrt(
    ...
    cos_h = torch.sum(h_edges[:, 0:grid_h - 1, :, :] * h_edges[:, 1:grid_h, :, :], 3) / (
                torch.sqrt(torch.sum(h_edges[:, 0:grid_h - 1, :, :] * h_edges[:, 0:grid_h - 1, :, :], 3)) * torch.sqrt(
...
def cal_distortion_term(init_mesh, target_mesh, overlap, grid_w=8, grid_h=8):
    ...
    error = torch.matmul(
        torch.matmul(torch.matmul(A, torch.inverse(torch.matmul(A.permute(0, 2, 1), A))), A.permute(0, 2, 1)), V) - V
