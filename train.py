...
def parse_args():
    ...
    parser.add_argument('--loss-type', type=str, default='8terms',
                        choices=['8terms', '6terms', '4terms', '2terms+', '2terms', 'l1', 'l2'],
    ...
    parser.add_argument('--GRID_W', default=16, type=int, help='mesh resolution of width(default: 8)')
    parser.add_argument('--GRID_H', default=12, type=int, help='mesh resolution of height(default: 6)')
    ...
    parser.add_argument('--lam_perception', default=5e-6, type=float, help='weight of perception loss (default: 5e-6)')
...
class Trainer(object):
    ...
    # --------------------------------- Training of deeprect ----------------------------
    def training(self, epoch):
        train_loss_epoch = []
        # if self.args.loss_type == '3terms' or self.args.loss_type == '2terms':
        #     raise ValueError(
        #         '=> Loss function: %s is not supported, please set the loss correctly' % self.args.loss_type)

        if self.args.loss_type in ['8terms', '6terms', '4terms', '2terms+', '2terms']:
            appearance_loss_epoch = []
        if self.args.loss_type in ['8terms', '6terms', '4terms']:
            perception_loss_epoch = []
        if self.args.loss_type in ['8terms', '6terms', '2terms+']:
            mask_loss_epoch = []
        if self.args.loss_type in ['8terms']:
            mesh_loss_epoch = []
        ...
        for i, sample in enumerate(tbar):
            ...
            if self.args.loss_type == '8terms':
                g_loss, appearance_loss, perception_loss, mask_loss, mesh_loss = self.criterion(train_mesh_primary,
                                                                                                ...
                                                                                                self.args.lam_perception,
                                                                                                self.args.lam_mask,
                                                                                                self.args.lam_mesh
                                                                                                )
            elif self.args.loss_type == '6terms':
                g_loss, appearance_loss, perception_loss, mask_loss = self.criterion(
                    train_warp_image_primary,
                    train_warp_mask_primary,
                    train_warp_image_final,
                    train_warp_mask_final,
                ...
                g_loss, appearance_loss, perception_loss = self.criterion(
                    train_warp_image_primary,
                    train_warp_mask_primary,
                    train_warp_image_final,
                    train_warp_mask_final,
            ...
            elif self.args.loss_type == '2terms+':
                g_loss, appearance_loss, mask_loss = self.criterion(
                    train_warp_image_primary,
                    train_warp_mask_primary,
                    train_warp_image_final,
                    train_warp_mask_final,
                ...
                g_loss, appearance_loss = self.criterion(
                    train_warp_image_primary,
                    train_warp_mask_primary,
                    train_warp_image_final,
                    train_warp_mask_final,
            ...
            elif self.args.loss_type == 'l1' or self.args.loss_type == 'l2':
                g_loss = self.criterion(train_warp_image_final, train_gt)
            ...
            if self.args.loss_type in ['8terms', '6terms', '4terms']:
                perception_loss_epoch.append(perception_loss.item())
            if self.args.loss_type in ['8terms', '6terms', '2terms+']:
        ...
        if self.args.loss_type in ['8terms', '6terms', '4terms']:
            self.writer.add_scalar('train/perception_loss_epoch', np.mean(perception_loss_epoch), epoch)
            myprint(self.logging, '     Vgg Loss : ({:.7f} * {:.7f} = {:.7f})'.format(np.mean(perception_loss_epoch),
                                                                                      self.args.lam_perception,
                                                                                      np.mean(
                                                                                          perception_loss_epoch) * self.args.lam_perception))
        if self.args.loss_type in ['8terms', '6terms', '2terms+']:
